...
export const ResellerRegistrationApprovedNotify: FC<ServerPageProps> = async (props) => {
	...
	const { OrgApproved, UserApproved } = (
		await getTypedLocalization(cache, context.locale as string, 'EmailTemplate')
	).Reseller;
	const label = flowType === FLOW_IDENTIFIER.orgAdmin ? UserApproved : OrgApproved;
