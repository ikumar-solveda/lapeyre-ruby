...
export class Promotion<SecurityDataType = unknown> {
	...
	promotionGetPromotionById = (
		...
		if (!this.traceDetails || this.traceDetails.includes('promotionGetPromotionById')) {
	...
	promotionGetPromotionsByQuery = (
		...
		if (!this.traceDetails || this.traceDetails.includes('promotionGetPromotionsByQuery')) {
			const paramsLogger = logger.child({
				params,
				query: query ?? {},
				body: null ?? {},
